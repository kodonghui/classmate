...
3. 아래 순서로 Classmate를 설치한 뒤 패널의 **Claude Code 연결하기**를 누릅니다.
...
1. 패널 상단 **Claude Code 연결하기**를 누릅니다.
...
![공식 로그인 연결과 별도 답변 테스트 — 합성 계정 상태로 실제 Chromium에서 촬영](docs/screenshots/connection-0.1.4.png)
...
| 전사는 오지만 AI 답이 없음 | 패널 상단 **Claude Code 연결하기** → 공식 로그인 → **답변 테스트 · 소량 사용**. 실패 상태의 해결 방법 확인. 설치 등록 문제가 남으면 `Diagnose.cmd` 실행 |
...
**패널의 연결 점검**은 공식 Claude Code의 버전·로그인 상태를 확인합니다. 로그인 필요 시 **Claude Code 연결하기**를 누르세요. 공식 창이 열리지 않을 때만 PowerShell에서 `claude auth login`을 실행합니다. 같은 PC의 Windows 사용자 계정으로 로그인해야 합니다. 공식 Claude 웹/일반 앱 로그인이나 다른 PC의 계정은 자동으로 가져오지 않습니다.
