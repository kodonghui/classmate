...
**[Windows 0.1.6 설치 ZIP 받기](https://github.com/kodonghui/classmate/releases/download/v0.1.6/Corthex-Classmate-0.1.6-windows-x64.zip)** · **[버전별 다운로드](https://github.com/kodonghui/classmate/releases)** · **[문제 제보](https://github.com/kodonghui/classmate/issues)**

> 현재 배포판: **0.1.6 · Windows x64 · Chrome · Claude Code**
>
...
1. **내 PC에 Claude Code 설치** → 아래 **Windows 0.1.6 ZIP** 설치 → Chrome에 확장을 추가합니다. **설정 → Claude 연결 → Claude Code 연결하기**로 본인 공식 로그인을 완료합니다. 자세한 경로는 [설치하기](#3-classmate-설치하기)를 참고하세요.
...
5. 패널 설정에서 **사용 목적·학습 묶음 이름**을 정하고 **새 묶음 시작**을 누릅니다. 전환 후 해당 Tiro 공유 탭을 새로고침하고 영상 탭에서 아이콘을 다시 누르세요. 전사 문장을 누르거나 하단에 질문하면 **그 문단 바로 아래에 답변**이 나타납니다.
...
3. 아래 순서로 Classmate를 설치한 뒤 **설정 → Claude 연결 → Claude Code 연결하기**를 누릅니다.
4. 로그인 필요 시 열리는 **공식 Claude Code 창과 브라우저**에서 본인 계정으로 로그인합니다. 이미 로그인되어 있으면 재로그인하지 않습니다.
5. 패널이 로그인 상태를 자동 확인하고 수업 연결을 갱신합니다. 로그인 확인만으로 AI 답변 성공을 보장하지는 않습니다.
6. **답변 테스트 · 소량 사용**을 직접 눌러 실제 응답도 확인하세요. 수업 전사·화면 없이 짧은 고정 질문만 보내며 본인 AI 사용량을 소비합니다.

공식 창이 열리지 않을 때만 PowerShell에서 `claude auth login`을 실행한 뒤 연결하기를 다시 눌러 보세요. `claude auth status --text`로 상태를 확인할 수도 있습니다. **설치 폴더로 이동해서 로그인할 필요는 없습니다.** 같은 Windows 사용자 환경과 같은 Claude Code 실행 파일을 사용하는 것이 중요합니다.
...
1. 위의 **Windows 0.1.6 설치 ZIP 받기**를 누릅니다.
2. 파일 이름이 `Corthex-Classmate-0.1.6-windows-x64.zip`인지 확인합니다.
...
```text
%LOCALAPPDATA%\Programs\CorthexClassmate\0.1.6\app\extension
```
...
6. **Corthex Classmate**, 버전 **0.1.6**, 사용 스위치 켜짐을 확인합니다.
...
### 6-0. 설정에서 Claude를 연결합니다

패널 오른쪽 위 **설정 아이콘 → Claude 연결**을 여세요. 연결 점검·로그인·답변 테스트는 모두 이곳에 있습니다. 정상 상태의 수업 화면에는 연결 영역이 나타나지 않습니다. 연결에 문제가 있을 때만 **연결 설정** 안내가 한 줄 표시됩니다.

1. **설정 → Claude 연결 → Claude Code 연결하기**를 누릅니다.
2. 로그인이 필요하면 공식 창이 열립니다. 본인이 계정을 선택하고 인증을 마치세요. 공식 창에서 코드를 요구하면 **그 창에만** 입력합니다. Classmate나 개발자에게 보내지 마세요.
3. Classmate가 약 3초 간격으로 최대 5분 동안 로그인 상태를 확인합니다. 로그인이 확인되면 수업용 연결을 새로 엽니다. **진행 중인 질문·저장이 있으면 끊지 않고 완료 후 재시도를 안내합니다.**
4. **답변 테스트 · 소량 사용**을 누르면 수업 자료 없이 짧은 실제 응답을 시험합니다. 자동 실행되지 않으며 Claude 계정 사용량이 발생합니다.
5. 성공 문구가 나오면 아래에서 학습 묶음을 설정하고 질문하세요.

![공식 로그인 연결과 별도 답변 테스트 — 합성 계정 상태로 실제 Chromium에서 촬영](docs/screenshots/connection-0.1.6.png)

- **연결 점검:** 설치·로그인 상태만 읽습니다. AI 답변을 생성하지 않습니다.
- **Claude Code 연결하기:** 이미 로그인되어 있으면 다시 로그인하지 않고 수업 연결을 갱신합니다.
- **자동 확인 중단:** 로그인 완료를 기다리는 것만 중단합니다. 공식 로그인 창은 직접 닫아 주세요. 로그아웃하거나 공부 기록을 삭제하지 않습니다.
- **답변 테스트:** 짧은 응답 확인이며, 수업 질문의 품질·남은 사용 한도·모든 화면 기능의 성공을 보장하지 않습니다.
- 버튼이 비활성화되고 업데이트 안내가 나오면 **확장만이 아니라 새 ZIP의 로컬 도우미도 설치**해야 합니다.
- 로그인이 성공해도 사용 한도·네트워크·환경 설정 때문에 답변이 실패할 수 있습니다. 무한히 기다리는 대신 실패 상태와 해결 방법을 표시합니다. 오류 문구만 알려 주셔도 됩니다.
...
| 학습 묶음 이름 | `04~06_Spring_framework`, `민법 물권변동`, `기획회의`처럼 직접 정한 이름 |
| 새 묶음 시작 | 새로운 강의 묶음이나 회의를 시작할 때 사용 |
| 현재 이름 변경 | 지금 공부하는 묶음의 제목 변경. 문답·화면 기록은 보존 |
| 선택한 묶음 이어서 공부 | 다음 날에도 같은 묶음에 기록을 누적 |
| 단축키 Alt+S | 켜 두면 마지막 문장을 붙잡는 데 사용 가능 |

![0.1.3 학습 묶음 설정 화면 — 합성 자료로 실제 Chromium에서 촬영](docs/screenshots/study-bundles.png)

**영상 04·05·06을 하나의 학습 묶음으로 모을 수 있습니다.** 묶음은 날짜나 교시가 아니라 사용자가 선택한 이름으로 구분합니다. 새 묶음/이름 변경/이어서 공부를 누르면 패널을 다시 읽어 해당 기록을 표시합니다. 사용 목적은 새 묶음에 적용됩니다.

다른 묶음으로 바꾸기 전 영상·Tiro를 잠시 멈추세요. 묶음을 고른 뒤 **해당 Tiro 공유 탭을 새로고침하고, 참고할 영상 탭에서 Classmate 아이콘을 다시 누릅니다.** 여러 Tiro 공유 페이지를 동시에 열어 두지 마세요. 영상 제목만 보고 묶음을 자동 전환하지 않습니다. 기존 녹음본 하나에 04~06을 담았다면 그 Tiro 노트를 해당 묶음에 계속 연결해 사용하세요.

이름은 폴더명에도 쓰입니다. Windows에서 허용하지 않는 문자와 예약명(CON 등), 끝의 점·공백은 사용할 수 없습니다. 같은 이름의 다른 묶음은 새로 만들지 않고 기존 묶음을 선택하도록 안내합니다. 답변 생성 중에는 묶음 변경을 기다려 주세요.
...
| 이전 대화 | 같은 학습 묶음의 최근 완료 문답 최대 6쌍, 전체 약 12,000자 예산. 모든 문답을 매번 AI에 보내는 것은 아님 |
...
**설정 → 블로그 자료 저장**을 누르세요. **학습 마침 · 자료 저장**은 자료를 저장하고 Classmate의 화면 캡처와 연결된 Tiro 전사 관찰을 멈춥니다. 영상 재생이나 Tiro 녹음 자체는 직접 정지하세요.

기본 저장 위치는 `C:\Users\내이름\Documents\Classmate-Blog\내가 정한 묶음 이름`입니다. 실제 경로는 설정 화면과 저장 완료 안내에 표시됩니다. 고급 설치에서는 로컬 도우미의 `CLASSMATE_BLOG_DIR` 환경변수로 다른 루트를 지정할 수 있습니다.

```text
Classmate-Blog/
└─ 04~06_Spring_framework/
   ├─ README.md
   ├─ 자료/
   │  ├─ 학습자료.md          # 최신 전사 + 질문·답변 + 화면 연결
   │  ├─ 블로그작성요청.md    # Cowork 등에 전달할 요청
   │  ├─ session-bundle.json # 전체 판본과 상세 연결
   │  └─ frames/            # 실제로 저장된 화면만
   └─ output/               # 내가 작성·수정하는 글
```

같은 묶음으로 저장을 반복하면 **자동 생성 자료와 README만 갱신**합니다. **output의 글은 덮어쓰거나 삭제하지 않습니다.** 이름을 바꾸면 다음 저장 때 기존 폴더를 새 이름으로 옮기며 output도 함께 보존합니다. 다른 사람이 만든 동명 폴더나 다른 묶음 폴더가 있으면 덮어쓰지 않고 멈춥니다. 원본 SQLite와 화면의 내부 이름은 제목 변경으로 바뀌지 않습니다.

Cowork를 열어 **그 묶음 폴더만 연결**하고 `자료/블로그작성요청.md`를 읽어 작업해 달라고 요청하세요. 자료 갱신은 블로그 자동 집필이나 Notion/Tistory 자동 발행이 아닙니다. 원본 sessions 폴더를 수정·삭제하도록 맡기지 마세요. 사본을 AI 도구에 연결하면 선택한 서비스로 내용이 전송될 수 있습니다.
...
0.1.4 이하 설치기의 숨김 폴더 검사 오류입니다. **0.1.6 ZIP을 새로 내려받아 전부 압축 해제한 뒤 Install.cmd를 실행하세요.** 폴더 이름·숨김 설정·백신·회사 보안 정책을 변경할 필요가 없습니다. 이 오류는 프로그램 복사·등록 전 검사에서 발생하므로 실패했던 폴더를 직접 삭제하거나 Disconnect.cmd를 실행하지 마세요. 이미 정상 설치된 이전 버전이 있다면 아래 업데이트 절차를 따릅니다.
...
| 전사는 오지만 AI 답이 없음 | **설정 → Claude 연결 → Claude Code 연결하기** → 공식 로그인 → **답변 테스트 · 소량 사용**. 실패 상태의 해결 방법 확인. 설치 등록 문제가 남으면 `Diagnose.cmd` 실행 |
| 제목을 바꿨는데 현재 기록은 그대로 | 설정에서 현재 묶음을 선택한 뒤 이름 변경 저장. 새 묶음 시작과 기존 이름 변경은 별개 |
...
**설정의 연결 점검**은 공식 Claude Code의 버전·로그인 상태를 확인합니다. 로그인 필요 시 **Claude Code 연결하기**를 누르세요. 공식 창이 열리지 않을 때만 PowerShell에서 `claude auth login`을 실행합니다. 같은 PC의 Windows 사용자 계정으로 로그인해야 합니다. 공식 Claude 웹/일반 앱 로그인이나 다른 PC의 계정은 자동으로 가져오지 않습니다.

**Diagnose.cmd**는 설치·파일·경로·등록을 확인하는 보조 도구입니다. 설치 때 반드시 눌러야 하는 버튼은 아닙니다. 로그인 상태는 설정의 연결 점검으로 확인하고, 실제 답변·사용 한도·네트워크는 마지막 질문 왕복으로 확인하세요. 응답이 없으면 보통 60초, 대기열 연장 시 질문 시작 후 최대 120초에 멈춤 안내를 표시하며 뒤늦게 답이 오면 반영합니다.
...
**0.1.6는 숨김 AppData 폴더에서 설치가 중단되는 문제를 수정한 버전입니다. Chrome 새로고침만으로 새 파일이 다운로드되지는 않습니다.**
...
3. 새 0.1.6 ZIP을 전체 압축 해제하고 **Install.cmd**를 실행합니다.
4. Chrome을 다시 시작하고 확장 관리에서 새로 설치된 **0.1.6의 app\extension**을 로드합니다.
...
3. 버전 **0.1.6**인지 확인합니다. 숫자 버전 뒤의 `+g…`는 해당 빌드의 소스 식별자입니다.
