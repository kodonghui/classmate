...
**[Windows 0.1.0 설치 ZIP 받기](https://github.com/kodonghui/classmate/releases/download/v0.1.0/Corthex-Classmate-0.1.0-windows-x64.zip)** · **[버전별 다운로드](https://github.com/kodonghui/classmate/releases)** · **[문제 제보](https://github.com/kodonghui/classmate/issues)**

> 현재 배포판: **0.1.0 · Windows x64 · Chrome · Claude Code**
>
...
![실제 0.1.0 실행 화면: 위 전사, 아래 공부 대화](docs/screenshots/classroom-0.1.0.png)
...
A. PC 영상·회의 소리 ── Tiro 데스크톱 ──┐
                                      ├─ Tiro 공유 전사 페이지 ── Classmate 위쪽 전사
B. 현장 목소리 ────── 휴대폰 Tiro ─────┘
...
3. 시작 메뉴에서 **PowerShell**을 열고 `claude`를 입력해 실행합니다.
4. 화면에 나오는 로그인 절차를 **본인 계정**으로 완료합니다.
5. 짧은 질문으로 Claude Code가 실제로 답하는지 확인하고 종료합니다.
...
Claude와 Tiro의 요금제·한도는 별개입니다. Classmate가 구독 제한을 없애 주지는 않습니다.

## 3. Classmate 설치하기

### 3-1. ZIP 전체를 받습니다

1. 위의 **Windows 0.1.0 설치 ZIP 받기**를 누릅니다.
2. 파일 이름이 `Corthex-Classmate-0.1.0-windows-x64.zip`인지 확인합니다.
...
```text
%LOCALAPPDATA%\Programs\CorthexClassmate\0.1.0\app\extension
```
...
6. **Corthex Classmate**, 버전 **0.1.0**, 사용 스위치 켜짐을 확인합니다.
7. Chrome 오른쪽 위 퍼즐 모양 **확장 프로그램** 메뉴에서 Corthex Classmate의 **핀**을 눌러 고정합니다.

![격리된 Chrome에서 확인한 Classmate 확장 상세 화면](docs/screenshots/manual-extension-0.1.0.png)
...
이 단계가 끝나면 **왼쪽: 영상·자료 / 오른쪽 위: 전사 / 오른쪽 아래: AI 대화**로 사용합니다.
...
![실제 설정 화면](docs/screenshots/manual-settings-0.1.0.png)
...
2. Classmate 위쪽 **선생님 전사**에도 같은 내용이 보이는지 확인합니다.
3. 아래 입력창에 **“방금 내용을 처음 배우는 사람도 이해하게 설명해 주세요”**를 쓰고 Enter를 누릅니다.
4. 아래 대화에 질문과 답이 나타나는지 확인합니다.
...
선생님 말씀만 읽는 공간입니다. 모르는 **강조 단어**를 누르거나 **문장**을 누르면 질문을 시작합니다. 문장 선택 후 후보가 나오면 궁금한 부분을 고르세요. 예전에 질문한 대목의 **대화 보기**를 누르면 아래의 해당 대화로 이동합니다.

### 아래쪽: 나의 공부 대화

내가 물은 내용, AI의 설명, 그림이 모입니다. 다른 문단을 질문해도 완료된 대화는 같은 세션에 남습니다.

- **바로 묻기**: 자유롭게 질문합니다.
- **이 대화에 이어 묻기**: 이전의 특정 문단 설명을 선택해서 질문합니다. 입력창 위에 `이어서 질문 · …`이 표시되는지 확인하세요.
- **새 질문 / 해제**: 이전 문단 선택을 풀고 최근 수업 맥락으로 돌아갑니다.
- **질문한 순간의 전사**: 그 질문이 어디에서 나왔는지 다시 확인합니다.
- **Enter** 전송 / **Shift+Enter** 줄바꿈.
- **대화 넓게**: 설명·그림을 읽을 때 아래 공간을 넓힙니다. **수업도 함께**로 되돌립니다.
- 가운데 경계선을 위아래로 끌면 비율을 바꿀 수 있습니다. 키보드로 경계선에 초점을 두고 ↑/↓로도 조절합니다.
- 이전 내용을 읽는 중 새 내용이 오면 **현재 수업으로 돌아가기 / 새 답변으로 이동**으로 따라갑니다.

기본 비율은 **전사 약 60% / 대화 약 40%**이며 조절한 값은 기억합니다. 두 영역의 스크롤은 독립적입니다.
...
![실제 스윔레인 예제와 대화 공간](docs/screenshots/manual-diagram-0.1.0.png)
...
| 전사는 오지만 AI 답이 없음 | 같은 PC PowerShell에서 `claude`가 실제로 답하는지 → 본인 로그인·한도 → `Diagnose.cmd` 순서로 확인 |
| 제목을 바꿨는데 현재 기록은 그대로 | 제목 변경은 기존 기록 이름 바꾸기가 아님. 다음 세션 시작 후 상단 제목 확인 |
| 오래전 문답을 기억하지 못함 | 해당 문단의 **대화 보기 → 이 대화에 이어 묻기**, 또는 질문에 핵심 인용 |
| 그림 대신 글만 옴 | 그림 종류 선택 → 그림 요청 → Enter. 표시 오류 시 텍스트 대체 설명 확인 |
| 버전이 예전 그대로 | 확장과 로컬 도우미 모두 업데이트했는지 확인. 단순 새로고침은 파일 다운로드가 아님 |

**Diagnose.cmd**는 설치·파일·경로·등록을 확인합니다. 로그인이나 실제 AI 응답 성공까지 보장하는 검사는 아닙니다.
...
**0.0.x → 0.1.0은 확장과 로컬 도우미가 함께 바뀝니다. Chrome 새로고침만으로 업데이트되지 않습니다.**

1. 진행 중인 질문을 마치고 Chrome 확장 관리에서 Classmate를 **사용 중지**합니다.
2. **이전 버전 설치 폴더의 Disconnect.cmd**로 이전 연결을 해제합니다.
3. 새 0.1.0 ZIP을 전체 압축 해제하고 **Install.cmd**를 실행합니다.
4. Chrome을 다시 시작하고 확장 관리에서 새로 설치된 **0.1.0의 app\extension**을 로드합니다.
...
0.1.0은 자동 테스트·실제 Chromium UI 시험·Windows 패키지 검사를 거칩니다. 화면 예제는 합성 자료입니다. 깨끗한 별도 PC의 전체 설치, 실제 Claude의 다이어그램 생성 품질, 하루 8교시 장시간 사용을 모두 보장하는 것은 아닙니다. **첫날에는 위의 1분 연결 시험부터 진행해 주세요.**
